...
import { DunkTokenAbi } from "@/wallet/dunkToken/index"; //contract
...
const View: React.FC<ViewProps> = ({ onClose }) => {
	...
	const [contractData, setContractData] = useState({
		...
		chainId: "421614",
		imgurl: boxGoldImg, //盒子图片
		priceImg: iconDunk, //价格图片
		name: "CHRONO CUBE II", //合约名字
		price: "100", //价格
		priceUnit: "Dunk", //单位
	...
	const mintBlindBox = async () => {
		if (!isLoading) {
			...
			if (GameItemContract.items.TokenAddress !== ethers.ZeroAddress) {
				// 判断dunk地址是否正确
				if (GameItemContract.items.TokenAddress == import.meta.env.VITE_CONTRACT_DUNKTOKEN_ADDR) {
					// dunk余额是否充足
					const dunkBalance = Number(ethers.formatUnits(DunkTokenContract.items.balance, 6)); //dunk 余额
					const unitPrice = Number(ethers.formatUnits(GameItemContract.items.price, 6)); //购买单价
					...
					if (dunkBalance >= mintPrice) {
						...
						const allowDunk = await DunkTokenContract.DunkTokenProvider.allowance(
							...
							contractData.contract, //GameItem-address
						);
						...
						// mint调用
						await GameItemContract.GameItemSigner.mint(contractData.tokenId, count)
							.then(async (res: any) => {
								setIsLoading(false); //Loading
